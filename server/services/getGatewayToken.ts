...
export default async function getGatewayToken(req: Request, res: Response) {
    try {
        ...
        logger.debug(`(service.getGatewayToken) mac: ${mac}, gatewayInfoList: ${JSON.stringify(gatewayInfoList)}, i: ${i}`);
        if (i === -1) {
        ...
        if (!gatewayInfo.tokenValid) {
            ...
            if (tokenRes.error === -1) {
                return res.json(toResponse(1202));
            }
            ...
            if (updateIndex === -1) {
                return res.json(toResponse(1203));
            }
